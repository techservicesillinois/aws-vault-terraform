...
resource "aws_db_instance" "vault_storage_mariadb" {
    ...
    multi_az = length(data.aws_subnet.private) > 1
    vpc_security_group_ids = [
        aws_security_group.vault_storage_mariadb[count.index].id
    ]
    db_subnet_group_name = aws_db_subnet_group.vault_storage_mariadb[count.index].id
    ...
    monitoring_role_arn = var.enhanced_monitoring ? null : data.aws_iam_role.rds_monitoring.arn
    monitoring_interval = var.enhanced_monitoring ? null : 60
